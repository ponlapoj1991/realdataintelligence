...
import { LEGACY_LOCALSTORAGE_KEY_DISCARDED_DB, LOCALSTORAGE_KEY_DISCARDED_DB } from '@/configs/storage'
...
const databaseNamePrefix = 'RealPPTX'
const legacyDatabasePrefix = 'PPTist'
...
export const deleteDiscardedDB = async () => {
  ...
  const localStorageDiscardedDB =
    localStorage.getItem(LOCALSTORAGE_KEY_DISCARDED_DB) ||
    localStorage.getItem(LEGACY_LOCALSTORAGE_KEY_DISCARDED_DB)
  ...
  const discardedDBNames = databaseNames.filter(name => {
    if (name.indexOf(databaseNamePrefix) === -1 && name.indexOf(legacyDatabasePrefix) === -1) return false
    
    const [prefix, id, time] = name.split('_')
    if ((prefix !== databaseNamePrefix && prefix !== legacyDatabasePrefix) || !id || !time) return true
  ...
  localStorage.removeItem(LEGACY_LOCALSTORAGE_KEY_DISCARDED_DB)
}
